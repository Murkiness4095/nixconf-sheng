# sheng —— 小米平板 6S Pro 的设备侧 NixOS 配置
...
| 路径 | 作用 |
| --- | --- |
| `configuration.nix` | `nixosConfigurations.sheng`：装配各模块，并覆盖平台默认值（hostname、时区、字体、zram、swap、`systemPackages`） |
| `user.nix`、`ssh.nix` | 账号、密码哈希、sudo、SSH 公钥 |
| `sheng-programs.nix` | hjem 用户软件集合（浏览器 / 聊天 / 媒体 / 工具），唯一的包入口 |
| `core.nix`、`hjem.nix`、`fastfetch/`、`shell/` | hjem 接线、fastfetch、fish + starship |
| `niri/`、`noctalia/` | niri 会话与 Noctalia 状态栏，含各自的配置文件 |
| `theme.nix`、`power.nix` | GTK/图标主题、电源与亮度 |
| `alacritty.nix`、`kitty.nix`、`fcitx5.nix`、`ibus.nix` | 终端与输入法 |
| `vscode/`、`zed/` | 编辑器配置 |
| `librewolf.nix`、`nixpak/` | 自定义包：LibreWolf AppImage 封装、qq/wechat 沙箱封装 |
...
`sheng-programs.nix` 中安装）；`diff` 依赖 `nvd`。
